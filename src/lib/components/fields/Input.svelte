  ...
  function validate() {
    ...
    if (validationRules.characterSets) {
      validationRules.characterSets?.forEach((rule) => {
        if (rule.exc) {
          ...
          if (matches.length > 0) {
            errors.push(
              `Must not include any character(s) from: ${rule.chars}`,
            );
          }
        } else {
          ...
          if (matches.length < (rule.min || 1)) {
            errors.push(
              `Must include at least ${rule.min || 1} character(s) from: ${rule.chars}`,
            );
          }
        }
      });
    ...
    if (validationRules.pattern) {
      ...
      if (!regex.test(value)) {
        ...
        const symbolsMatch = patternString.match(
          /(?!{\d+(?:,\d+)?}|\$\/|\?=\.|\[A-Z\])[^a-zA-Z\d\s]/g,
        );
        if (symbolsMatch) {
          ...
          if (!symbolMatch) {
            patternErrors.push(
              `Must include at least one symbol from: ${symbols}`,
            );
          }
        }
        ...
        if (excludedCharsMatch) {
          ...
          if (excludedMatch) {
            patternErrors.push(
              `Must not include any character(s) from: ${excludedChars}`,
            );
          }
        }
